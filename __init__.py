...
def _ctx(uid=None, role=None):
    """Inject cc, nc, wl into all buyer templates."""
    uid  = uid  or session.get('user_id')
    role = role or session.get('role', '')
    ctx  = {'nc': models.unread_count(uid) if uid else 0}
...
def home():
    ...
    return render_template('buyer/home.html',
        ...
        wallet=models.get_wallet(uid), **_ctx(uid))
    ...
    if request.method == 'POST':
        ...
        if payment_method == 'card':
            ...
            if not transaction_id or len(transaction_id) < 5:
                flash('Enter the transaction ID after payment.', 'error')
                return redirect(url_for('buyer.checkout', mode=mode, pid=pid, qty=qty))
        ...
        payment_status = 'paid' if payment_method in ('card', 'qr', 'wallet') else 'pending'
        ...
        for item in items:
            ...
            oid = models.create_order(
                ...
                item['pid'], item['name'], item.get('image', ''),
            ...
            if auction_time_str:
                try:
                    ...
                    if len(parts) == 3:
                        ...
                        auction_hours = max(1, total_secs / 3600)
                    elif len(parts) == 2:
                        h, m = int(parts[0]), int(parts[1])
                        auction_hours = max(1, h + m / 60)
                    else:
                        auction_hours = int(auction_time_str)
                except Exception:
                    auction_hours = 24
            else:
                auction_hours = int(request.form.get('auction_hours', 24))
